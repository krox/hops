...
TEST_CASE("complex arithmetic", "[hops]")
{
    ...
    hops::mul<std::complex<float>>(dOut.view(), {2.5f, 0.0f}, dX.view(),
                                   dY.view());
    hops::add_mul<std::complex<float>>(dOut.view().step(2), {-1.0f, 0.0f},
    ...
    {
        auto expected =
            (i % 2 == 0 ? std::complex{1.5f, 0.0f} : std::complex{2.5f, 0.0f}) *
            hX[i] * hY[i];
